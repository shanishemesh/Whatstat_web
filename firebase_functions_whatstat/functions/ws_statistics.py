...
#                             count top 10 emojis
    ...
    return group_name, user_names, user_message_counts, message_count, user_voice_counts, user_media_counts, user_emoji_counts, media_sent, voice_messages_sent
...
def sentiment_analysis(local_file_path):
# Open the file and read the content
    with open(local_file_path, "r", encoding="utf-8") as file1:
    ...
    print("Most Positive User:", most_positive_user, "Average Sentiment:", user_average_sentiment[most_positive_user])

    # Find the most negative user
    most_negative_user = min(user_average_sentiment, key=user_average_sentiment.get)
    print("Most Negative User:", most_negative_user, "Average Sentiment:", user_average_sentiment[most_negative_user])
        

    print(f'Total negative score: {negative_score/len(negative_sentences)}')
    print(f'Total positive score: {positive_score/len(positive_sentences)}')

    print(f'Total negative messages: {len(negative_sentences)}')
    print(f'Total positive messages: {len(positive_sentences)}')
...
def calculate_mean_first_message_time(file_path):
...
def calculate_mean_last_message_time(file_path):
    with open(file_path, 'r') as file:
...
def detect_earliest_sender(file_path):
...
def detect_latest_sender(file_path):
    latest_sender = None
    latest_time = None
...
def calculate_day_frequency(file_path):
    with open(file_path, 'r') as file:
...
def calculate_by_day_frequency(file_path):
    with open(file_path, 'r') as file:
        day_frequency = defaultdict(int)
